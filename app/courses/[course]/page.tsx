...
import { CourseHomeBandActions } from "@/components/course-home-band-actions";
...
export default async function CourseHomePage({ params }: { params: Promise<{ course: string }> }) {
  ...
  const course = getCourseHome(slug);
  if (!course) notFound();
  const written = writtenProgressCounts(course.nav);
    ...
    <main id="main-content">
      <div className="ih-band ih-course-home-band px-4 py-6 sm:px-8 lg:px-12">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between lg:gap-12">
          ...
          <CourseHomeBandActions
            slug={course.slug}
            lessonCount={lessonCount}
            projectCount={projectCount}
            startHref={course.startHref}
          />
        </div>
      </div>
