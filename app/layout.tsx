...
import { getChapterRequirements, getProgressLookup } from "@/lib/content";

export const metadata: Metadata = {
  metadataBase: new URL("https://interview-help.vercel.app"),
  title: { default: "Quarry", template: "%s · Quarry" },
  description: "Free tutorial pages for Quarry roadmaps: one section at a time, with the matching chapter project on its own page.",
  openGraph: { title: "Quarry", description: "Learn one section, then complete that chapter's project.", type: "website" },
};
...
STORY: A learner lands, reads the offer, picks a course card, then follows one section and that chapter's project.
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  const progressLookup = getProgressLookup();
  const chapterRequirements = getChapterRequirements();
  return (
    <html lang="en" suppressHydrationWarning>
      <body>
        ...
        <ProgressProvider lookup={progressLookup} requirements={chapterRequirements}>
