...
type ReaderProps = { title: string; markdown: string; sourcePath: string; headings: Heading[]; slug?: string; overviewHref?: string };

function PhaseNavigation({ headings }: { headings: Heading[]; slug?: string }) {
...
export function ReaderLayout({ title, markdown, sourcePath, headings, slug, overviewHref }: ReaderProps) {
  const phases = headings.filter((heading) => heading.phase !== undefined);
    ...
    <main id="main-content" className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 lg:py-8">
      ...
      <div className="grid min-w-0 gap-8 lg:grid-cols-[250px_minmax(0,760px)_190px] lg:justify-center xl:grid-cols-[270px_minmax(0,800px)_210px]">
        <aside className="hidden lg:block"><div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto pr-3">
          {overviewHref && <Link href={overviewHref} className="mb-5 inline-flex items-center gap-2 text-xs font-medium text-muted hover:text-ink"><AppIcon name="previous" size={13} /> Course overview</Link>}
          ...
          <div className="mt-6 space-y-1 border-t hairline pt-4 text-xs text-muted"><Link href="/projects" className="flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-ink/5 hover:text-ink"><AppIcon name="project" size={ICON_SIZE.compact} /> Projects</Link><Link href="/interview" className="flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-ink/5 hover:text-ink"><AppIcon name="interview" size={ICON_SIZE.compact} /> Interview practice</Link></div>
        </div></aside>
        <div className="min-w-0 rounded-2xl border hairline surface px-5 py-7 shadow-soft sm:px-8 lg:px-10 lg:py-10"><MarkdownDocument markdown={markdown} sourcePath={sourcePath} progressScope={slug} />
          <nav className="mt-16 flex flex-wrap items-center justify-between gap-3 border-t hairline pt-6 text-sm" aria-label="Document navigation">{overviewHref ? <Link href={overviewHref} className="inline-flex items-center gap-2 text-muted hover:text-ink"><AppIcon name="previous" size={15} /> Overview</Link> : <span />}<Link href="/courses" className="inline-flex items-center gap-2 font-medium">Explore courses <AppIcon name="expand" size={15} /></Link></nav>
        </div>
        <aside className="hidden xl:block"><div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto"><h2 className="mb-4 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.12em] text-muted"><AppIcon name="resource" size={13} /> On this page</h2><ActiveOutline headings={outline} /></div></aside>
      </div>
    </main>
  );
}
