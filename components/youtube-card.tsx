    ...
    <span className="my-4 block overflow-hidden rounded-xl border hairline surface not-prose">
      {playing ? <span className="relative block aspect-video bg-black"><iframe className="absolute inset-0 size-full" src={embed} title={label} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></span> : (
        <button onClick={() => setPlaying(true)} className="group relative block aspect-video w-full overflow-hidden bg-slate-900 text-left">
          {info.videoId ? <img src={`https://i.ytimg.com/vi/${info.videoId}/hqdefault.jpg`} alt="" loading="lazy" className="size-full object-cover opacity-75 transition duration-300 group-hover:scale-[1.02] group-hover:opacity-90" /> : <span className="absolute inset-0 grid place-items-center text-slate-500"><AppIcon name="playlist" size={ICON_SIZE.heading} /></span>}
          <span className="absolute inset-0 grid place-items-center"><span className="grid size-14 place-items-center rounded-full bg-white text-slate-950 shadow-xl transition group-hover:scale-105"><AppIcon name="run" size={ICON_SIZE.nav} weight="fill" /></span></span>
