...
import { parseCourseMarkdown } from "../lib/parse-course.js";
...
for (const sec of projSections) {
  ...
  const projectTitle = projTitleMatch ? projTitleMatch[1].replace(/\*\*/g, "").replace(/^["']|["']$/g, "").trim() : heading;
  const problemStatement = probMatch ? probMatch[1].trim() : "";
  const checklist = checklistMatch ? checklistMatch[1].trim() : "";
  
  // Determine artifact kind for schema compliance:
  // code | lab | design | analysis | runbook | rehearsal
  let kind = "code";
...
for (let i = 0; i < course.phases.length; i++) {
  ...
  for (const l of regularLessons) {
    out += `- **${l.id} — ${l.title}:** Apply ${l.title} to ${p.projectTitle} and verify the result.\n`;
  }
  out += `\n#### SPECIFICATION\n\n`;
  out += `- **Format:** ${p.format}\n`;
...
console.log("Successfully wrote content/projects/computer-science.md");
